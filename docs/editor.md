...
- **Router:** `src/admin/lib/routing/` — in-house router replacing `react-router-dom`. Workspace/page routes are wrapped in a per-route `<ErrorBoundary>` and `<Suspense>`, with root redirects plus a final `path="/admin/*"` catch-all redirecting unknown admin URLs to `/admin/site` instead of rendering an empty tree. Unauthenticated visitors are redirected to Logto (`/admin/api/cms/auth/login`), not an in-app form. Public-site 404s are NOT claimed — the publish pipeline's NotFound handling owns those.
...
- **Workspaces:** `site` (the editor — the admin home), `content`, `data`, `media`, `ai`, `account`. `plugins`/`pluginPage` remain valid workspace types but the workspace is hidden (its routes redirect to `/admin/site`; the plugin engine is untouched). User/role management is gone — identities come from Logto (see [features/auth-and-access.md](features/auth-and-access.md)). Capability-gated by `canAccessWorkspace`.
...
`siteLiveOrigin` is written once by `useAdminBoot` from the `/me` session (`currentSite.liveOrigin`, derived server-side by `siteLiveOrigin()` in `server/publish/requestSite.ts`: the site's custom domain when set, else `https://<slug>.<PUBLIC_BASE_DOMAIN>` when a base domain is configured, else `null`). A site switch reloads the page, so boot is the only writer. On per-host installs the admin origin serves only the default site, so the button must open the absolute URL; when per-host serving is unconfigured the origin stays `null` and the button falls back to the same-origin relative path.
...
└── pages/                      ← workspace implementations
    ...
    ├── account/                ← read-only identity + sign out (identity managed in Logto)
    └── ...
```
...
42px-wide vertical strip on the far left. Primary navigation panels sit in the top group. Each button gets an automatic rail tint from its full panel identity, with repeats avoided inside the visible rail group, and opens a panel in the left sidebar. Implementation: `src/admin/pages/site/sidebars/PanelRail/PanelRail.module.css`.

The primary rail group is, in order: **Explorer** (`database-solid` icon, `gold` accent — carried over from the standalone Layers rail button it replaced), **Framework**, **Selectors**. The **Dependencies** rail button and the bottom **AI assistant** global group are intentionally **hidden** — their panels (`DependenciesPanel` / `AgentPanel`) and store state (`dependenciesPanelOpen` / `isAgentOpen`, capability `canUseAiChat`) stay intact, they just have no rail affordance (`GLOBAL_RAIL_ITEMS` is empty and the `dependencies` entry is dropped from `PRIMARY_RAIL_ITEMS`). Read-only callers (Viewer / Client) see only the Explorer rail button — the structural Framework / Selectors panels are dropped from both the rail and their panel mounts.

### Left sidebar

Opens the rail-selected panel:

- `ExplorerPanel` — the consolidated navigation panel. One `<Panel>` shell hosting a top `SegmentedControl` with **Layers / Site / Code** tabs (default **Layers**). Each tab renders the corresponding panel in its headerless `tab` variant — the ExplorerPanel shell owns the chrome (header + tabs + close):
  ...
  - The **Media** tab is intentionally hidden from the Explorer (the media library is reached through its own workspace). `MediaExplorerPanel` stays used by the Media/Content workspaces.
  - Active tab is held in `explorerPanelTab` (`uiSlice`, `'layers' | 'site' | 'code'`) and persisted per-workspace via `siteEditorLayoutPersistence` (a persisted `'media'` value normalizes back to `'layers'`).
...
The Framework panel's **Type** tab stores Google/custom font assets and editable font tokens together under `site.settings.fonts`. **Google fonts load from the Google Fonts CSS2 CDN** via `<link>` tags (`buildGoogleFontsLinkTags` / `buildGoogleFontsHref` in `@core/fonts`) — nothing is downloaded to disk, so published sites stay free of font binaries. **Custom** (media-library) fonts still emit self-hosted `@font-face` rules. Font tokens own the builder-facing variables such as `--font-primary`, the assigned font asset, and the fallback stack. Removing a font is a metadata-only edit (there is no delete endpoint). The property-panel `font-family` control is a rich picker: token rows write `var(--font-primary)` so the selected node or class keeps following future token swaps, direct font rows write a concrete family stack as an escape hatch, and the text input still accepts manual values.
...
- `OpenLivePageButton` (`src/admin/shared/OpenLivePageButton/`) — toolbar icon (always visible, not Site-editor-only) that opens the live site in a new tab. Target URL is `adminUi.siteLiveOrigin` (the site's public origin — custom domain or `<slug>.<PUBLIC_BASE_DOMAIN>` — from `/me`; empty when per-host serving is unconfigured) + `adminUi.activeLivePath` (active document's public path when an editor is open, site root `/` otherwise). Tooltip changes between "Open live page" (active path) and "Open live site" (null). Component stays outside `src/admin/pages/site/` so it mounts on every admin route without touching the editor graph.
...
- Source-of-truth files:
  ...
  - `src/admin/state/adminUi.ts` — cross-shell Zustand store (settings modal, site-import modal, site name/favicon, activeLivePath, siteLiveOrigin)
  ...
  - `src/admin/pages/site/panels/ExplorerPanel/ExplorerPanel.tsx` — consolidated navigation panel (Layers / Site / Code tabs; the Media tab is hidden); owns the shell + `SegmentedControl`, renders DomPanel / SiteExplorerPanel (one instance for both Site+Code tabs via `sectionGroup`) in their headerless `tab` variant
