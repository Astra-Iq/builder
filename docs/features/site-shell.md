...
`fonts` is the installed font library and font-token contract (`src/core/fonts/`). `fonts.items` is the installed font library: **Google** entries carry no files and load from the Google Fonts CSS2 CDN via `<link>` tags; **custom** entries are self-hosted, media-backed font files. `fonts.tokens` is the editable builder-facing contract: each token owns a stable CSS variable such as `font-primary`, an optional assigned `FontEntry` id, and a fallback stack. The publisher emits those as `:root { --font-primary: "Family", sans-serif; }`; editor controls should prefer `font-family: var(--font-primary)` over raw family names when the design should follow future font swaps.
