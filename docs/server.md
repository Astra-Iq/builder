...
| `requestSite.ts`                  | `resolveSiteForRequest(db, host)` — visitor request → `site_id` before the per-site slot read. Default site for now; the Host/custom-domain lookup seam. |
| `publishStorageRegistry.ts` / `publishPush.ts` | Object-storage push: registry of `PublishStorageAdapter`s (local-disk no-op default; plugins register S3/R2) + `pushPublishedSite` shipping the swapped slot keyed `sites/<siteId>/…`. |
