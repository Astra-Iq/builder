...
async function seedRow(db: DbClient, id: string, slug: string, status = 'draft'): Promise<void> {
  await db`
    insert into data_rows (id, site_id, table_id, cells_json, slug, status, author_user_id, created_by_user_id, updated_by_user_id)
    values (${id}, ${'default'}, ${'components'}, ${{ name: id }}, ${slug}, ${status}, ${USER_ID}, ${USER_ID}, ${USER_ID})
  `
}
...
describe('applyDataRowChanges — table scoping', () => {
  it('never soft-deletes a row belonging to ANOTHER table', async () => {
    ...
    await db`
      ...
      values ('post-row', 'default', 'posts', ${{ title: 'Post' }}, 'a-post', 'published')
    `
    await seedRow(db, 'vc-a', 'card')
